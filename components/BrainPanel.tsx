...
function MessageBubble({ message }: { message: Message }) {
  ...
  const bgColor = isUser ? 'bg-gray-100 text-gray-900' : isClaude ? 'bg-amber-50 text-amber-900 border border-amber-100' : isSystem ? 'bg-green-50 text-green-900 border border-green-200' : 'bg-blue-50 text-blue-900 border border-blue-100'
  ...
  const timestamp = message.timestamp.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' })
  ...
  return (
    <div className={`flex ${alignment}`}>
      <div className={`${maxWidth} ${bgColor} rounded-xl px-3 py-2`}>
        ...
        {message.thinking && <p className="text-[10px] italic opacity-50 mb-1">{message.thinking}</p>}
