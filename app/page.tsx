...
export default function Home() {
  ...
  const runCommand = async (command: string) => {
    ...
    try {
      ...
      if (result.stdout) {
        result.stdout.split('\n').forEach((line) => {
          if (line.trim()) addTerminalLine('stdout', line)
        })
      }

      if (result.stderr) {
        result.stderr.split('\n').forEach((line) => {
          if (line.trim()) addTerminalLine('stderr', line)
        })
      }
  ...
  const handleUserMessage = async (message: string) => {
    ...
    // If no sandbox, connect first
    if (!sandboxId) {
      setStatus('executing')
      await connectSandbox()
    }

    // Simulate Claude processing (placeholder for actual LLM integration)
    setStatus('claude-coding')
    addMessage('claude', `I'll help you with: "${message}"\n\nLet me set up the environment...`)

    // Demo: Run a simple command to show sandbox is working
    await runCommand('echo "Sandbox is ready!" && node --version && npm --version')

    setStatus('gemini-auditing')
    addMessage(
      'gemini',
      'I\'ve reviewed the setup. The sandbox environment is configured correctly with Node.js and npm available. Ready to proceed with development.'
    )
  ...
  return (
    <main className="flex flex-col h-screen bg-jam-bg">
      {/* Header */}
      <header className="flex items-center justify-between px-6 py-4 border-b border-jam-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-white">Project Jam</h1>
            <p className="text-xs text-zinc-500">
              Claude builds • Gemini supervises
            </p>
          </div>
        </div>
        ...
        <div className="w-1/2 border-r border-jam-border overflow-hidden">
          <BrainPanel
            ...
            status={
              status === 'idle'
                ? 'Waiting for input'
                : status === 'claude-coding'
                ? 'Claude is thinking...'
                : status === 'gemini-auditing'
                ? 'Gemini is reviewing...'
                : 'Processing...'
            }
          />
      ...
      <CommandBar
        ...
        placeholder={
          sandboxId
            ...
            : 'Type to connect sandbox and start building...'
        }
      />
    </main>
  )
